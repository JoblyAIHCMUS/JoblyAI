...
describe('withdrawApplicationHandler', () => {
  it('withdraws application when status is APPLIED', async () => {
    const findUnique = vi.fn().mockResolvedValue({
      id: 1, candidateId: 'user-123', status: 'APPLIED',
    });
    const update = vi.fn().mockResolvedValue({
      id: 1, status: 'WITHDRAWN', job: { id: 1 }, resume: { id: 1 },
    });
    const state = buildState(findUnique, update);

    const result = await withdrawApplicationHandler(state, { applicationId: 1 });

    expect(update).toHaveBeenCalledWith({
      where: { id: 1 },
      data: { status: 'WITHDRAWN' },
      include: {
        job: { include: { category: true, company: true, postedBy: { select: { id: true, name: true, email: true } } } },
  ...
  it('rejects withdrawal when status is not APPLIED', async () => {
    const findUnique = vi.fn().mockResolvedValue({
      id: 1, candidateId: 'user-123', status: 'INTERVIEW',
    });
    const update = vi.fn();
    const state = buildState(findUnique, update);
    ...
    expect(result.content[0].text).toBe('Only applications with APPLIED status can be withdrawn');
  });

  it('returns "Application not found" for cross-user access', async () => {
    const findUnique = vi.fn().mockResolvedValue({
      id: 1, candidateId: 'other-user', status: 'APPLIED',
    });
    const update = vi.fn();
    const state = buildState(findUnique, update);
