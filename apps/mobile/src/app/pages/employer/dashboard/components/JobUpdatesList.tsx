        ...
        <View key={job.id} className="bg-white rounded-3xl p-5 border border-[#CBD5E1] shadow-sm mb-4">
          <View className="flex-row justify-between items-start mb-4">
            ...
            <View className="px-3 py-2 bg-[#EBF9F1] rounded-full">
              <Text className="font-medium text-sm text-[#56CDAD]">Full-Time</Text>
            </View>
          </View>

          <Text className="text-xl font-bold text-[#0F172A] mb-1">{job.title}</Text>
          ...
          <View className="flex-row gap-x-2 mb-4">
            {job.tags.map((tag, i) => (
              <View key={i} className="border border-[#7C8493] px-3 py-2 rounded-full">
                <Text className="font-medium text-[#7C8493] text-sm">{tag}</Text>
              </View>
            ))}
          </View>
          ...
          <View>
            <Text className="text-[#0F172A] font-bold mb-2 text-base">
              {job.applied} applied <Text className="font-normal text-[#475569] text-base">of {job.capacity} capacity</Text>
            </Text>
            <View className="h-2.5 bg-[#CBD5E1] rounded-full overflow-hidden">
              <View className="h-full bg-[#22C55E] rounded-full" style={{ width: `${(job.applied / job.capacity) * 100}%` }} />
            </View>
          </View>
        </View>
      ))}
