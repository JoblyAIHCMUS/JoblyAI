...
import { Eye, Pencil, Send, X, Trash2, ArrowLeftCircle } from 'lucide-react-native';
...
export const JobCardMenu: React.FC<JobCardMenuProps> = ({
  ...
  const getMenuItems = () => {
    ...
    if (status === 'Draft') {
      baseItems.push({ label: 'Publish Job Posting', icon: Send, onPress: onPublishJobPosting, color: '#0F172A' });
      baseItems.push({ label: 'Mark as Closed', icon: X, onPress: onMarkAsClosed, color: '#0F172A' });
    } else if (status === 'Live') {
      baseItems.push({ label: 'Revert to Draft', icon: ArrowLeftCircle, onPress: onRevertToDraft, color: '#0F172A' });
      baseItems.push({ label: 'Close Job Posting', icon: X, onPress: onMarkAsClosed, color: '#0F172A' });
    } else if (status === 'Closed') {
      baseItems.push({ label: 'Revert to Draft', icon: ArrowLeftCircle, onPress: onRevertToDraft, color: '#0F172A' });
      baseItems.push({ label: 'Reopen Job Posting', icon: Send, onPress: onPublishJobPosting, color: '#0F172A' });
    }

    baseItems.push({ label: 'Delete', icon: Trash2, onPress: onDelete, color: '#DC2626' });
    return baseItems;
  };
              ...
              <View className="bg-white rounded-2xl px-4 py-2 shadow-2xl border border-[#E2E8F0]">
                {menuItems.map((item, index) => (
                  ...
                  >
                    ...
                    <Text className={`text-base font-medium ${item.color === '#DC2626' ? 'text-[#DC2626]' : 'text-[#0F172A]'}`}>
                      {item.label}
                    </Text>
                  </TouchableOpacity>
                ))}
