...
export default function MyApplicationsPage() {
  ...
  const firstName = profile?.firstName || profile?.name?.split(' ')[0] || getGreetingName(user) || 'there';
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [isBannerVisible, setIsBannerVisible] = useState(true);
  const [activeFilterTab, setActiveFilterTab] = useState<ApplicationFilterTab>('ALL');
  ...
  const filteredApplications = useMemo(() => {
    ...
    return applications.filter((application) => {
      ...
      const matchesCompany = appliedCompany
        ? application.company.toLowerCase().includes(appliedCompany.toLowerCase())
        : true;

      const matchesLocation = appliedLocation
        ? application.location.toLowerCase().includes(appliedLocation.toLowerCase())
        : true;

      return matchesDate && matchesTab && matchesSearch && matchesCompany && matchesLocation;
    });
  }, [activeFilterTab, appliedDateRange, appliedCompany, appliedLocation, searchQuery, applications]);
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count;
  }, [activeFilterTab, activePreset, appliedCompany, appliedLocation, searchQuery]);
  ...
  }, [draftCompany, draftDateRange.from, draftDateRange.to, draftJobType, draftLocation]);
          ...
          ListEmptyComponent={
            loading ? (
              ...
              <ApplicationsEmptyState
                activeTab={activeFilterTab === 'ACTIVE' ? 'APPLIED' : activeFilterTab === 'CLOSED' ? 'REJECTED' : 'ALL'}
                searchQuery={searchQuery}
              />
            )
          }
