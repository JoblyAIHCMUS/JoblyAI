import { useQuery } from '@tanstack/react-query';
import {
  getCandidatePreShortlist,
  type PreShortlistApplicationView,
} from '../api/preShortlist';

const QUERY_KEY = (applicationId: number) =>
  ['candidate-pre-shortlist', applicationId] as const;

/**
 * Mirrors apps/web/src/api-hook/pre-shortlist/useCandidatePreShortlist.ts.
 * Fetches the candidate's pre-shortlist view: questions, existing answers,
 * evaluation status, and overall suggestion.
 */
export function useCandidatePreShortlist(applicationId: number) {
  const query = useQuery<PreShortlistApplicationView, Error>({
    queryKey: QUERY_KEY(applicationId),
    queryFn: () => getCandidatePreShortlist(applicationId),
    enabled: Number.isFinite(applicationId) && applicationId > 0,
    staleTime: 30 * 1000,
    retry: 1,
  });

  return {
    data: query.data ?? null,
    isLoading: query.isLoading,
    isError: query.isError,
    error: query.error,
    refetch: query.refetch,
  };
}
