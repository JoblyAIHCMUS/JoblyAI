    ...
    <div className="relative rounded-[10px] border border-[#CBD5E1] bg-white overflow-hidden flex flex-col items-end pb-6">
      ...
      <div className="absolute left-8 top-[70px]">
        <div className="relative w-[140px] h-[140px]">
          ...
          <img src={candidate.avatar} alt="avatar" className="absolute w-[140px] h-[140px] rounded-full object-cover" />
        </div>
      </div>
      ...
      <div className="w-full pl-[180px] pr-8 mt-6">
        <div className="flex flex-row justify-between items-start">
          <div className="flex flex-col gap-2">
            <div className="text-2xl font-semibold text-[#0F172A] font-['Lexend_Deca']">{candidate.name}</div>
            ...
            {candidate.openForOpportunities && (
              <div className="mt-2 px-6 py-3 bg-[#CCFBF1] rounded-lg flex items-center gap-2">
                ...
                <span className="text-[#14B8A6] font-semibold text-base">OPEN FOR OPPORTUNITIES</span>
              </div>
            )}
          </div>
          <button className="px-6 py-3 rounded-[5px] border border-[#CBD5E1] text-[#4338CA] font-semibold text-base font-['Lexend_Deca'] hover:bg-[#EEF2FF]">Edit Profile</button>
        </div>
      </div>
    </div>
  );
