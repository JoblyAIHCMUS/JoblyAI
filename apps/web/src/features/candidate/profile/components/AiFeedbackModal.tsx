...
export function AiFeedbackModal({
  ...
  const [activeTab, setActiveTab] = useState<'audit' | 'rewriter' | 'methodology'>('audit');
  ...
  const getRuleReference = (ruleName: string) => {
    ...
    if (lowerName.includes('harvard') || lowerName.includes('hbs')) return CV_AUDIT_RULES.harvard_verbs;
    return null;
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription className="text-sm text-slate-500 mt-0.5">
                Detailed evaluation based on global recruitment standards and actionable writing feedback.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
            ...
            {isLegacy && (
              <div className="space-y-6">
                ...
                <div className="space-y-3 p-5 border rounded-2xl border-slate-100 bg-white shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <p className="text-sm text-slate-600 leading-relaxed">
                    {feedback?.formatting || 'Evaluation of layout and readability.'}
                  </p>
                </div>
              </div>
            )}
                ...
                {activeTab === 'audit' && feedback.auditReport && (
                  <div className="space-y-6">
                    ...
                    <div className="p-5 bg-amber-50/40 border border-amber-100/50 rounded-2xl text-slate-700 text-sm leading-relaxed flex gap-3.5 shadow-sm">
                      ...
                      <div>
                        <span className="font-bold text-amber-900 block mb-1">Evaluation Scope Notice:</span>
                        This review focuses strictly on your resume's <strong className="font-semibold text-slate-900">writing content quality</strong>—specifically measuring <strong className="font-semibold text-slate-900">quantitative outcomes</strong> (Google XYZ) and <strong className="font-semibold text-slate-900">active vocabulary</strong> (Harvard Action Verbs). Since the system analyzes raw extracted text, visual layout structures, physical margins, font styles, or graphical bullet points are not evaluated.
                      </div>
                    </div>
                    ...
                    {feedback.generalAdvice && (
                      <div className="p-6 bg-indigo-50/40 border border-indigo-100/50 rounded-2xl text-slate-700 text-[15px] leading-relaxed flex gap-4 shadow-sm">
                        ...
                        <div>
                          <span className="font-bold text-indigo-955 block mb-1.5 text-base">Executive Summary</span>
                          {feedback.generalAdvice}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 gap-6">
                      {Object.entries(feedback.auditReport).map(([key, item]) => {
                            ...
                            <div className="p-6 space-y-5 text-[15px]">
                              ...
                              {ref && (
                                <div className="p-4 bg-slate-50/80 border border-slate-100 rounded-xl text-sm space-y-2 relative z-10">
                                  ...
                                  <p className="text-slate-700 leading-relaxed">{ref.summary}</p>
                                  <div className="pt-1 flex">
                                    ...
                                    >
                                      ...
                                      <span>Official Reference: {ref.label} &rarr;</span>
                                    </button>
                                  </div>
                                </div>
                              )}
                    ...
                    {(feedback.detailedStrengths?.length || feedback.detailedWeaknesses?.length) && (
                      <div className="space-y-8 pt-2">
                        {/* Detailed Strengths - Emerald Stripe Left */}
                        {feedback.detailedStrengths && feedback.detailedStrengths.length > 0 && (
                          <div className="space-y-4">
                            ...
                            <div className="space-y-3.5">
                              {feedback.detailedStrengths.map((str, i) => (
                                ...
                                >
                                  <div className="font-bold text-slate-900 text-[15px] tracking-tight">{str.title}</div>
                                  <p className="text-sm text-slate-650 leading-relaxed">{str.description}</p>
                                  {str.evidence && (
                                    <div className="bg-slate-50/80 p-3 rounded-lg border border-slate-100 text-xs text-slate-500 italic">
                                      <span className="font-semibold text-slate-450 not-italic block text-[10px] uppercase tracking-wider mb-0.5">Evidence from Resume:</span>
                                      "{str.evidence}"
                                    </div>
                                  )}
                                </div>
                        ...
                        {feedback.detailedWeaknesses && feedback.detailedWeaknesses.length > 0 && (
                          <div className="space-y-4">
                            <h4 className="font-bold text-slate-800 text-base flex items-center gap-2 px-1">
                              ...
                              {feedback.detailedWeaknesses.map((weak, i) => (
                                ...
                                >
                                  <div className="space-y-1.5">
                                    <div className="font-bold text-slate-900 text-[15px] tracking-tight">{weak.title}</div>
                                  ...
                                  <p className="text-sm text-slate-650 leading-relaxed">{weak.description}</p>
                                  {weak.evidence && (
                                    <div className="bg-rose-50/15 p-3 rounded-lg border border-rose-100/30 text-xs text-slate-500 italic">
                                      <span className="font-semibold text-rose-450 not-italic block text-[10px] uppercase tracking-wider mb-0.5">Found in Resume:</span>
                                      "{weak.evidence}"
                                    </div>
                                  )}
                                </div>
                ...
                {activeTab === 'rewriter' && (
                  <div className="space-y-6">
                    <div className="p-6 bg-indigo-50/40 border border-indigo-100/50 rounded-2xl text-slate-700 text-[15px] leading-relaxed flex gap-4 shadow-sm">
                      ...
                      <div>
                        <span className="font-bold text-indigo-955 block mb-1 text-base">Writing Suggestions</span>
                        Compare the sentences below. Swap out weak phrasings in your CV for these high-impact rewrites to instantly align with top recruitment standards.
                      </div>
                    </div>
                    ...
                    <div className="space-y-5">
                      {feedback.rewriteSuggestions && feedback.rewriteSuggestions.length > 0 ? (
                          ...
                          >
                            <div className="px-5 py-3 border-b border-slate-50 bg-slate-50/20 flex justify-between items-center text-sm">
                              <span className="font-semibold text-slate-505">
                                Standard: <span className="text-indigo-650 font-bold">{item.ruleApplied}</span>
                              </span>
                            </div>
                            <div className="p-5 space-y-4">
                              ...
                              <div className="pt-3 border-t border-slate-50 text-sm text-slate-500 flex items-start gap-2 leading-relaxed">
                                ...
                                <span>
                                  <span className="font-bold text-slate-700">Review Rationale: </span>
                                  {item.explanation}
                                </span>
                              </div>
                            </div>
                        ...
                        <div className="text-center py-12 bg-white border border-dashed border-slate-200 rounded-2xl text-slate-400 text-sm">
                          No rewrites needed! Your achievements are already strong and well-quantified.
                        </div>
                      )}
                    </div>
                  </div>
