...
import { Trash2, AlertTriangle, Briefcase, GraduationCap, Code2, Award, Share2, Phone, User, Info, FileX, Mail, Globe, ArrowRight } from 'lucide-react';
...
export function CvDeleteImpactModal({
  ...
  const getAffectedItems = () => {
    ...
    return {
      ...
      totalAffected: affectedSkills.length + affectedExperiences.length + affectedEducations.length + affectedCertificates.length + affectedContacts.length + affectedSocials.length
    };
  };

  const affected = getAffectedItems();

  const renderSectionHeader = (icon: React.ReactNode, title: string, colorClass: string) => (
    <div className={cn("flex items-center justify-between mb-3 border-b pb-1", colorClass)}>
  ...
  const Section = ({ icon, title, current, affectedItems, renderItem, colorClass }: any) => (
    <section className="mb-8 last:mb-0">
      <div className="flex gap-6">
        ...
        <div className="w-1/2">
          {renderSectionHeader(icon, `${title} to be removed`, "text-red-700 border-red-200")}
          <div className="space-y-3">
            {affectedItems?.length > 0 ? (
              affectedItems.map((item: any, i: number) => renderItem(item, i, true))
            ) : (
              <p className="text-[10px] text-slate-400 italic bg-slate-50 p-3 rounded-lg border border-dashed">No unique items from this CV.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <DialogContent className="max-w-[1100px] w-[95vw] h-[90vh] flex flex-col p-0 overflow-hidden border-none shadow-2xl">
        <DialogHeader className="px-10 py-6 border-b bg-red-50/40 shrink-0">
          <div className="flex items-center gap-3 text-red-600">
            ...
            <div>
              <DialogTitle className="text-2xl font-['Lexend_Deca'] text-slate-900 leading-none">Delete CV: {resumeName}</DialogTitle>
              <DialogDescription className="text-slate-500 mt-2 text-sm">
                Carefully review the changes below. Deleting this document will remove all profile data that was <span className="font-bold text-red-600">exclusively</span> extracted from it.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          <div className="flex shrink-0 bg-slate-100/80 border-b backdrop-blur-sm">
            <div className="w-1/2 px-10 py-3 text-[11px] font-bold uppercase tracking-wider text-slate-500 border-r flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-slate-400" /> Current Profile Status
            </div>
            <div className="w-1/2 px-10 py-3 text-[11px] font-bold uppercase tracking-wider text-red-600 bg-red-50/30 flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /> Deletion Impact (Unique Data)
            </div>
          </div>
          ...
          <ScrollArea className="flex-1 h-full min-h-0 px-10 py-8 bg-white">
            <div className="max-w-4xl mx-auto space-y-12">
              ...
              <Section 
                ...
                renderItem={(exp: any, i: number, isAffected: boolean) => (
                  <div key={i} className={cn(
                    ...
                    isAffected ? "border-red-200 ring-1 ring-red-50/50 scale-[1.02] shadow-red-100/50" : "border-slate-200"
                  )}>
                    <div className="flex justify-between items-start mb-1">
                      <div className={cn("font-bold text-sm tracking-tight", isAffected ? "text-red-900" : "text-slate-800")}>{exp.jobTitle}</div>
                    </div>
                    <div className={cn("text-xs font-semibold", isAffected ? "text-red-600" : "text-blue-600")}>{exp.companyName}</div>
                    {isAffected && <Badge className="mt-2 bg-red-100 text-red-700 hover:bg-red-100 text-[9px] border-red-200 uppercase tracking-tighter font-bold">Will be removed</Badge>}
                  </div>
                )}
              />
              ...
              <Section 
                ...
                renderItem={(edu: any, i: number, isAffected: boolean) => (
                  <div key={i} className={cn(
                    ...
                    <div className={cn("font-bold text-sm tracking-tight", isAffected ? "text-red-900" : "text-slate-800")}>{edu.school}</div>
                    <div className="text-xs text-slate-500 font-medium">{edu.degree} • {edu.fieldOfStudy}</div>
                    {edu.grade && <div className="text-[10px] text-slate-400 mt-1 italic">Grade: {edu.grade}</div>}
                    {isAffected && <Badge className="mt-2 bg-red-100 text-red-700 hover:bg-red-100 text-[9px] border-red-200 uppercase tracking-tighter font-bold">Will be removed</Badge>}
                  </div>
                )}
              />
              ...
              <Section 
                ...
                renderItem={(s: any, i: number, isAffected: boolean) => (
                  <div key={i} className={cn(
                    ...
                    isAffected ? "border-red-200 ring-1 ring-red-50/50 scale-[1.02]" : "border-slate-200"
                  )}>
                    <span className={cn("text-xs font-bold tracking-tight", isAffected ? "text-red-900" : "text-slate-800")}>{s.title}</span>
                    {isAffected ? (
                      <Badge className="bg-red-100 text-red-700 hover:bg-red-100 text-[9px] border-red-200 font-bold uppercase tracking-tighter">Remove</Badge>
                    ) : (
                      <Badge variant="secondary" className="text-[9px] h-5 bg-slate-100 text-slate-600 font-bold uppercase tracking-tighter">{s.level}</Badge>
                    )}
                  </div>
                )}
              />
              ...
              <Section 
                ...
                renderItem={(cert: any, i: number, isAffected: boolean) => (
                  <div key={i} className={cn(
                    ...
                    <div className={cn("font-bold text-sm tracking-tight", isAffected ? "text-red-900" : "text-slate-800")}>{cert.name}</div>
                    <div className="text-xs text-slate-500 font-medium">{cert.issuer}</div>
                    {cert.credentialId && <div className="text-[10px] text-slate-400 mt-1 font-mono uppercase tracking-tighter">ID: {cert.credentialId}</div>}
                    {isAffected && <Badge className="mt-2 bg-red-100 text-red-700 hover:bg-red-100 text-[9px] border-red-200 uppercase tracking-tighter font-bold">Will be removed</Badge>}
                  </div>
                )}
              />
              ...
              <div className="p-5 bg-red-50 border border-red-200 rounded-2xl flex items-start gap-4 shadow-sm">
                ...
                <div>
                  <h4 className="text-sm font-bold text-red-900 font-['Lexend_Deca']">Critical Action Required</h4>
                  <p className="text-xs text-red-700 leading-relaxed mt-1 font-medium">
                    This action is irreversible. Deleting this CV will permanently purge the {affected.totalAffected} data points highlighted above. Your professional profile and AI score will be automatically recalculated to reflect the remaining data sources.
                  </p>
                </div>
              </div>
            </div>
