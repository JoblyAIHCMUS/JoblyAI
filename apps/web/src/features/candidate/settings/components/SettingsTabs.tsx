      ...
      <TabsList className="self-stretch pl-6 bg-bg-primary shadow-[inset_0px_-1px_0px_0px_rgba(214,221,235,1.00)] rounded-none h-auto inline-flex justify-start items-start gap-10 p-0">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <div className={`justify-center text-base font-medium font-['Lexend_Deca'] leading-5 ${isActive ? 'text-text-primary' : 'text-text-secondary'}`}>
                {tab.label}
              </div>
              {isActive && <div className="self-stretch h-1 bg-icon-accent-primary" />}
            </button>
          );
        })}
      </TabsList>
