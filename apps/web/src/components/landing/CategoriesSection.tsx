      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-4 gap-8">
          {categories.slice(4).map((cat) => (
            <div key={cat.name} className="p-8 rounded-lg border border-slate-200 bg-white hover:shadow-lg transition">
              <div className="w-12 h-12 bg-indigo-100 rounded-md mb-8 flex items-center justify-center">
                <cat.icon className="w-6 h-6 text-indigo-600" />
              </div>
              <h3 className="text-2xl font-bold text-slate-900 mb-4">{cat.name}</h3>
              <div className="flex items-center gap-3">
                <p className="text-slate-600">{cat.jobs} jobs available</p>
