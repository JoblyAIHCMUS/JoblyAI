      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="md:hidden flex gap-4 overflow-x-auto pb-4 px-2 mx-2 snap-x snap-mandatory scrollbar-hide">
          {featuredJobs.map((job, idx) => (
            <div key={idx} className="min-w-[280px] w-[280px] p-5 border border-slate-200 rounded-lg bg-white hover:shadow-lg transition snap-start flex-shrink-0">
              ...
              <h3 className="text-lg font-bold text-slate-900 mb-2 line-clamp-2">{job.title}</h3>
              <p className="text-sm text-slate-600 mb-4">
                {job.company} • {job.location}
              </p>
              <p className="text-sm text-slate-500 mb-4 line-clamp-2">Lorem ipsum dolor sit amet...</p>
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
          {featuredJobs.map((job, idx) => (
            <div key={idx} className="p-5 md:p-6 border border-slate-200 rounded-lg bg-white hover:shadow-lg transition">
              <div className="w-12 h-12 bg-slate-300 rounded-full mb-4"></div>
              <div className="inline-block px-3 py-1 border border-indigo-600 text-indigo-600 text-xs font-semibold rounded mb-4">
                Full Time
              </div>
              <h3 className="text-lg md:text-xl font-bold text-slate-900 mb-2">{job.title}</h3>
              <p className="text-sm text-slate-600 mb-4">
                {job.company} • {job.location}
              </p>
