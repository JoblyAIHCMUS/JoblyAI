...
export default function FilterGroup({
  ...
  return (
    <div className="flex w-full flex-col gap-3">
      <button type="button" onClick={() => onToggleExpand(title)} className="flex items-center justify-between">
        <h3 className="text-base font-semibold leading-6 text-slate-900">{title}</h3>
        <ChevronDown className={`h-5 w-5 text-slate-700 transition-transform ${expanded ? 'rotate-180' : 'rotate-0'}`} />
      </button>
              ...
              >
                <span
                  className={`flex h-6 w-6 items-center justify-center rounded-[5px] border-2 ${
                    isChecked ? 'border-slate-300 bg-indigo-700' : 'border-slate-200 bg-white'
                  }`}
                >
