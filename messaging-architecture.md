...
6. **`useSendMessage` mutation** (`hooks/messaging/useSendMessage.ts`, 117 lines):
...
| `hooks/messaging/useSendMessage.ts` (117)                                                       | RQ `useMutation` with optimistic `local-{uuid}` insert, ack swap, 10s timeout; `onError` marks the optimistic message with `failed: true` (no toast) and `MessageBubble` renders a red `AlertCircle` + "Couldn't send" indicator under the failed bubble |
